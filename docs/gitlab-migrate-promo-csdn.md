...
> **摘要**：当 GitLab 实例版本过旧（如 8.x / 9.x），官方要求按版本逐级升级，路径长、风险高。本文介绍我开源的 Shell 工具 `gitlab-migrate`——通过「新装 GitLab + 数据迁移」的方式，自动化完成仓库 mirror 迁移、Group/个人项目区分处理、用户权限同步，支持断点续传与并行推送。

---

## 一、背景：为什么需要这个工具？

公司或团队里跑着一个 **GitLab 8.x / 9.x** 的老实例，操作系统老旧、PostgreSQL 版本跟不上，想升到 **GitLab 19.x** 却发现：
