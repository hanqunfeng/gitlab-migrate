...
本文档介绍在 **RHEL / CentOS / Rocky Linux / AlmaLinux** 等使用 `dnf` 的 RPM 系系统上安装 GitLab Community Edition（CE）。
...
将 `EXTERNAL_URL` 替换为你的 IP 或域名后执行：
...
安装过程会自动执行 `gitlab-ctl reconfigure`，首次配置可能需要数分钟，请耐心等待。
